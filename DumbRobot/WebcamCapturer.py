...
class WebcamCapturer:
    def __init__(self, device_index=0, capture_interval=5, images_folder_path="CapturedImages"):
        """
        device_index: The index of the webcam (0 for default camera)
        capture_interval: The number of seconds between captures
        images_folder_path: Path to the folder where captured images are stored
        """
        self.device_index = device_index
        self.capture_interval = capture_interval
        self.images_folder_path = images_folder_path
        os.makedirs(self.images_folder_path, exist_ok=True)  # Ensure folder exists
    ...
    def clear_old_images(self):
        # Delete all images in the images_folder_path
        for file_path in glob.glob(os.path.join(self.images_folder_path, "*")):
            os.remove(file_path)
    ...
    def capture_frame(self, output_prefix="frame", output_suffix="999", on_image_captured=None):
        ...
        output_path = os.path.join(self.images_folder_path, filename)
